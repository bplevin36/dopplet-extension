...
export default class XmlHttpRequest extends BaseFileSystem implements FileSystem {
   ...
   * --Temporarily set to 0 because chrome extension web accessible resources
   *   do not allow sending Content-Length header.  See imports also.
   */
  ...
  private _requestFileSizeSync(path: string): number {
    return 0; // getFileSizeSync(this.getXhrPath(path));
  }
}
